...
mongoose.connect(mongo, options).then(() => console.log('Brain: Successfully connected to MongoDB')).catch(err => console.error('MongoDB connection error:', err));
...
async function bloomCm(Bloom, message, fulltext, commands) {
    ...
    try { await commandModule.run(Bloom, message, fulltext, commands); }
    catch (err) { await Bloom.sendMessage(message.key.remoteJid, { text: '❗ An error occurred while executing the command.' }); }
}
...
async function checkCommandTypeFlags(Bloom, message) {
    try {
        ...
        const settings = await Settings.findOne({ group: groupId });
        if (!settings) return true;
        if (cmdData.type === 'game' && !settings.gameEnabled) { await Bloom.sendMessage(groupId, { text: '🎮 Games are disabled in this group.' }); return false; }
        if (cmdData.type === 'nsfw' && !settings.nsfwEnabled) { await Bloom.sendMessage(groupId, { text: '🔞 NSFW is disabled in this group.' }); return false; }
        return true;
    } catch (e) { return true; }
}
...
const bloomCmd = async (Bloom, message) => {
    try {
        ...
        const checks = [() => checkMode(Bloom, message), () => checkMessageType(Bloom, message), () => checkCommandTypeFlags(Bloom, message), () => checkAFK(Bloom, message)];
        let shouldProceed = true;
        for (const check of checks) { try { shouldProceed = shouldProceed && await check(); if (!shouldProceed) break; } catch (e) { shouldProceed = false; break; } }
        if (shouldProceed && command && commandRegistry[command]) await bloomCm(Bloom, message, fulltext, commandRegistry);
